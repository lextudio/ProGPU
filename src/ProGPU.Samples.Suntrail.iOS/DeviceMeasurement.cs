...
internal sealed class DeviceMeasurement
{
    ...
    public void Attach(GameView view, Window window)
    {
        ...
        _pipeline.EnableWorldPass = Environment.GetEnvironmentVariable("SUNTRAIL_WORLD_PASS") == "1";
        _pipeline.EnableSharedInstances = Environment.GetEnvironmentVariable("SUNTRAIL_SHARED_INSTANCES") == "1";
        _pipeline.EnableSceneInstances = Environment.GetEnvironmentVariable("SUNTRAIL_SCENE_INSTANCES") == "1";
        _pipeline.EnableSharedInstances |= _pipeline.EnableSceneInstances;
        _pipeline.EnableMaterialPages |= _pipeline.EnableWorldPass || _pipeline.EnableSharedInstances;
    ...
    private void Record(object? sender, double delta)
    {
        ...
        Console.WriteLine($"SUNTRAIL_MEASURE worldPass={_pipeline.EnableWorldPass} worldRenders={_pipeline.WorldRenderCount} worldBytes={_pipeline.WorldResidentBytes} materialBakes={_pipeline.MaterialBakeCount} materialBytes={_pipeline.MaterialResidentBytes} residentPages={_pipeline.MaterialResidentPages} visiblePages={_pipeline.MaterialVisiblePages} fallbackPages={_pipeline.MaterialFallbackPages} evictions={_pipeline.MaterialEvictions}");
        _window.WgpuContext!.TryCaptureNativeResourceSnapshot(out var native);
        Console.WriteLine($"SUNTRAIL_MEASURE sharedInstances={_pipeline.EnableSharedInstances} sceneInstances={_pipeline.EnableSceneInstances} staticSources={_pipeline.StaticSourceInstances} sharedSourceUploadBytes={_pipeline.SharedSourceUploadBytes} sharedPageUploadBytes={_pipeline.SharedPageUploadBytes} sharedUploadCalls={_pipeline.SharedUploadCalls} sharedCounterWindow=including-warmup");
