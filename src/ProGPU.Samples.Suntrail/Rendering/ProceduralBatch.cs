...
public enum Artwork { Sky, Cliff, Tree, Bush, Flower, Crate, Coin, Courier, Beetle, Lantern, Portal, Ledge, Thorns, Spark, Cloud, Mountain, Ruin, Mushroom, Shadow = 19, SunShaft, Fern, Grass, Crystal, Palm, Pine, Spire, Water, Cavern, Pipe, Saw, FlameJet, Crusher, Spring, Conveyor, Ice, Crumble }
...
public sealed class ProceduralBatch
{
    ...
    private readonly SpritePlacement[] _placements = new SpritePlacement[Capacity];
    public ReadOnlySpan<ProceduralSprite> Sprites => _sprites.AsSpan(0, Count);
    public ReadOnlySpan<SpritePlacement> Placements => _placements.AsSpan(0, Count);
    public Camera2D Camera { get; private set; } = new(Vector2.Zero, 1);
    ...
    private int[] _visiblePlatformIds = [];
    private int _visiblePlatformCount;
    ...
    public void Build(GameSession game, Vector2 size, float atmosphereTime)
    {
        ...
        Camera = new(new(_cameraX, _cameraY), _scale);
        Scene = new(atmosphereTime, game.Level.Biome, _cameraX, _scale);
        if (_visiblePlatformIds.Length < game.Level.Platforms.Length)
            Array.Resize(ref _visiblePlatformIds, (int)BitOperations.RoundUpToPowerOf2((uint)Math.Max(16, game.Level.Platforms.Length)));
        _visiblePlatformCount = game.Level.PlatformVisibility.QueryOrdered(
            new(_cameraX, _cameraY, worldWidth, size.Y / _scale), _visiblePlatformIds);
        float inset = Math.Max(8, 4 / _scale);
        OccluderCount = 0;
        if (EnableBackgroundOcclusion)
        {
            for (int visible = 0; visible < _visiblePlatformCount; visible++)
            {
                var platform = game.Level.Platforms[_visiblePlatformIds[visible]];
        ...
        for (int layer = 0; layer < 3; layer++)
        {
            ...
            for (int i = start; i < start + (int)(worldWidth / 290) + 5; i++)
            {
                ...
                AddLayer(Artwork.Mountain, i * 290, 625 - h, 470, h + 400,
                    new(parallax), new(1, 1, 1, 1), layer, seed);
            }
        }
        for (int i = -1; !cavern && i < 7; i++)
        {
            float x = i * 370 - (_cameraX * .09f + atmosphereTime * 3) % 370;
            AddScreen(Artwork.Cloud, x * _scale, (65 + Hash(i + 75) * 160) * _scale, 240 * _scale, 100 * _scale, new(1, 1, 1, .75f), dynamic: true);
        }
        ...
        for (int i = firstTree; i < firstTree + (int)(worldWidth / 225) + 3; i++)
        {
            ...
            AddLayer(landmark, i * 225, 640 - h, 235, h, new(.52f, .3f), Vector4.One, seed, 1);
        }
        if (cavern) AddScreen(Artwork.Cavern, 0, 0, size.X, 265 * _scale);
        if (biome is 3 or 6)
            AddScreen(Artwork.Water, 0, (690 - _cameraY * .2f) * _scale, size.X, 260 * _scale, dynamic: true);
        int ruinCell = (int)MathF.Floor(_cameraX * .25f / 1200);
        for (int i = ruinCell; i < ruinCell + 3; i++)
            AddLayer(Artwork.Ruin, i * 1200 + 900, 345, 230, 245, new(.25f, 0), new(1, 1, 1, .48f), i);
        if (biome == 1) {
            int archCell = (int)MathF.Floor(_cameraX * .34f / 240);
            for (int i = archCell; i < archCell + (int)(worldWidth / 240) + 2; i++)
                AddLayer(Artwork.Ruin, i * 240, 360, 330, 345, new(.34f, 0), new(1, 1, 1, .38f), i);
        }
        // Static world records are culled against the viewport before entering the batch.
        for (int visible = 0; visible < _visiblePlatformCount; visible++)
        {
            int platformIndex = _visiblePlatformIds[visible];
            var platform = game.Level.Platforms[platformIndex];
            ...
            if (platform.Kind == PlatformKind.Ground)
            {
                ...
                if (b.Width >= 320 && (b.X > 0 || biome != 0))
                    Add(landmark, b.X + 25, b.Y - 295, 240, 310, Vector4.One, seed);
                Add(Artwork.Cliff, b.X, b.Y - 8, b.Width, b.Height + 8, Vector4.One, seed);
                // Seed/index identity remains the original terrain-relative k.
                // The widest decoration reaches 32 jitter + 76 fern units right;
                // grass reaches 12 units left. Cull before hashing/record creation.
                var decorations = VisibleCellRange.Intersect(b.X + 16, 72,
                    (int)MathF.Ceiling(b.Width / 72), _cameraX, _cameraX + worldWidth, 12, 108);
                for (int k = decorations.Start; k < decorations.End; k++)
                {
                ...
                if (!IsDungeon && b.Width >= 320 && biome is 0 or 3 or 4 or 7) Add(Artwork.Bush, b.X + b.Width * .55f, b.Y - 67, 145, 76, Vector4.One, seed);
            }
            else
            {
                var art = platform.Kind switch
                {
                    PlatformKind.Pipe => Artwork.Pipe, PlatformKind.Crate => Artwork.Crate,
                    PlatformKind.Spring => Artwork.Spring, PlatformKind.Conveyor => Artwork.Conveyor,
                    PlatformKind.Ice => Artwork.Ice, PlatformKind.Crumble => Artwork.Crumble, _ => Artwork.Ledge
                };
                float warning = game.Level.PlatformWarning(platformIndex, game.Tick);
                float parameter = platform.Kind == PlatformKind.Conveyor ? platform.Contact.TangentSpeed :
                    platform.Kind == PlatformKind.Crumble ? warning : platform.Phase;
                float shake = warning * MathF.Sin(game.Time * 65) * 1.5f;
                Add(art, b.X - 3 + shake, b.Y - 5, b.Width + 6, b.Height + 12, Vector4.One, parameter,
                    dynamic: platform.Kind is PlatformKind.Moving or PlatformKind.Crumble);
            }
        }
        ...
        foreach (var mechanism in game.Level.Mechanisms)
        {
            ...
            Add(kind, b.X - 3, b.Y - 3, b.Width + 6, b.Height + 6, Vector4.One, mechanism.Cycle(game.Time), mechanism.IsDangerous(game.Time) ? 1 : 0, dynamic: true);
        }
        ...
        if (game.Level.Document is not null || !game.Level.IsDungeon) Add(Artwork.Portal, game.Level.Exit.X - 40, game.Level.Exit.Y - 188, 155, 205);
        foreach (var enemy in game.Level.Enemies)
        {
            if (enemy.Defeated) continue;
            float shadowY = enemy.Kind == EnemyKind.Walker ? enemy.Position.Y : enemy.AnchorY;
            Add(Artwork.Shadow, enemy.Position.X - 10, shadowY + 23, 66, 19,
                new(0, 0, 0, enemy.Kind == EnemyKind.Hoverer ? .14f : .28f), dynamic: true);
            Add(Artwork.Beetle, enemy.Position.X - 10, enemy.Position.Y - 12, 64, 51, Vector4.One,
                enemy.Speed < 0 ? -1 : 1, (float)enemy.Kind, dynamic: true);
        }
        if (game.Mode != GameMode.Fallen)
        {
            var p = game.Position;
            Add(Artwork.Shadow, p.X - 16, p.Y + 39, 66, 17, new(0, 0, 0, .3f), dynamic: true);
            Add(Artwork.Courier, p.X - 25, p.Y - 24, 82, 80,
                new(1, 1, 1, game.Invulnerability > 0 && game.Tick % 12 < 5 ? .42f : 1),
                game.Facing, game.Grounded ? Math.Clamp(Math.Abs(game.Velocity.X) / 180, 0, 1) : -1, dynamic: true);
        }
        foreach (var particle in game.Particles)
            if (particle.Life > 0) Add(Artwork.Spark, particle.Position.X - 14, particle.Position.Y - 14, 28, 28,
                new(1, 1, 1, Math.Clamp(particle.Life / particle.MaxLife, 0, 1)), particle.Kind, dynamic: true);
        ...
        for (int i = 0; i < 24; i++)
        {
            ...
            AddScreen(Artwork.Spark, x * _scale, y * _scale, 4 * _scale, 4 * _scale, new(1, 1, 1, .5f), biome == 5 ? 3 : biome == 4 ? 4 : 1, dynamic: true);
        }
    ...
    /// <summary>Original Suntrail courier art for source-geometry playtests. Position is already camera-rebased.</summary>
    public void BuildCourierPreview(Vector2 size, Vector2 position, float scale, float time, int facing, float stride)
    {
        if (size.X <= 0 || size.Y <= 0) return;
        Count = OccluderCount = 0; Size = size; IsDungeon = false;
        _scale = scale; _cameraX = _cameraY = 0; Camera = new(Vector2.Zero, 1);
        Scene = new(time, 0, 0, scale); Light0 = Light1 = Light2 = Vector4.Zero;
        AddScreen(Artwork.Shadow, position.X - 16 * scale, position.Y + 39 * scale, 66 * scale, 17 * scale,
            new(0, 0, 0, .3f), dynamic: true);
        AddScreen(Artwork.Courier, position.X - 25 * scale, position.Y - 24 * scale, 82 * scale, 80 * scale,
            Vector4.One, facing, stride, dynamic: true);
        Generation++;
    }

    private void Add(Artwork kind, float x, float y, float w, float h, Vector4 color = default, float p = 0, float q = 0, bool dynamic = false)
        => AddLayer(kind, x, y, w, h, Vector2.One, color, p, q, dynamic);
    private void AddLayer(Artwork kind, float x, float y, float w, float h, Vector2 cameraFactor, Vector4 color = default, float p = 0, float q = 0, bool dynamic = false)
    {
        var placement = SpritePlacement.World(new(x, y, w, h), cameraFactor, dynamic);
        AddPlaced(kind, Camera.ProjectBounds(placement), placement, color, p, q);
    }
    private void AddScreen(Artwork kind, float x, float y, float w, float h, Vector4 color = default, float p = 0, float q = 0, bool dynamic = false)
    {
        var bounds = new Vector4(x, y, w, h);
        AddPlaced(kind, bounds, SpritePlacement.Screen(bounds, dynamic), color, p, q);
    }
    private void AddPlaced(Artwork kind, Vector4 bounds, SpritePlacement placement, Vector4 color, float p, float q)
    {
        if (bounds.X + bounds.Z < 0 || bounds.Y + bounds.W < 0 || bounds.X > Size.X || bounds.Y > Size.Y) return;
        if (Count == Capacity) throw new InvalidOperationException("Suntrail procedural sprite capacity exceeded.");
        _placements[Count] = placement;
        _sprites[Count++] = new(bounds, color == default ? Vector4.One : color, new((float)kind, p, q, 0));
    }
