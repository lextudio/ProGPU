...
* Preserve `SKPoint` and `SKRect` as CPU-only `O(1)` value operations with no heap allocation or WebGPU initialization. Match native normalization, distance, reflection, arithmetic, formatting, half-open point containment, exact `Empty` identity, degenerate inclusive intersection, unconditional min/max union, standardized coordinates, and centered aspect fit/fill semantics. Point-based canvas overloads must remain thin delegates to the scalar implementations; do not fork command recording or rendering behavior.
...
The current baseline is 1,294 renderer tests and 149 headless tests. Update these counts only when tests are intentionally added or removed.
