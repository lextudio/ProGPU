...
* Keep virtualized indexed sources indexed. `ItemsControl` attachment to `IList` is O(1), and viewport realization is O(V) for V visible/overscan items. Do not eagerly enumerate/copy large glyph or data sources, allocate viewport scratch lists, or create binding delegates during measure, arrange, or scrolling.
...
* MotionMark and similar vector benchmarks must use public `DrawingContext` APIs with retained `PathGeometry`. Geometry generation may be O(N) when data or viewport changes, but steady animation must reuse segment objects, brushes, pens, HUD resources, and pooled group containers; frame update and recording stay O(N), with no per-segment managed allocation after warmup.
...
The current baseline is 1,331 renderer tests and 149 headless tests. Update these counts only when tests are intentionally added or removed.
